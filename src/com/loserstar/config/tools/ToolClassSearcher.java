package com.loserstar.config.tools;
...
public class ToolClassSearcher {
    ...
    protected static final Log LOG = Log.getLog(ToolClassSearcher.class);
    ...
    public static boolean isValiPkg(){
    	List<String> pkgs = Arrays.asList(LoserStarStringUtils.toString(PropKit.get("config.scan.package")).split(","));
    	if(pkgs.size() > 0){
    		return true;
    	}
        return false;
    ...
    private String classpath = PathKit.getWebRootPath() + File.separator + "WEB-INF" + File.separator + "classes";//PathKit.getRootClassPath();
